...
export const getAllJobsService = async (params) => {
  ...
  const filters = require("../Utils/filters.utils.js").buildFilters(filterParam, { numericFields: ["companyId", "salaryRange"], booleanFields: ["isRemote"] });
  if (search) {
    filters.OR = [
...
export const applyForJobService = async (jobId, applicantId, cvUrl) => {
  ...
  return prisma.application.create({ data: { jobId: id, applicantId, cvUrl, status: "PENDING" } });
};
